...
describe("embedded attempt harness pinning", () => {
  ...
  it("forwards runtime visibleSendPolicy into embedded attempts", async () => {
    const sessionEntry: SessionEntry = {
      sessionId: "visible-send-policy-session",
      updatedAt: Date.now(),
    };
    runEmbeddedAgentMock.mockResolvedValueOnce({
      meta: { durationMs: 1 },
    ...
    await runAgentAttempt({
      ...
      sessionFile: path.join(tmpDir, "session.jsonl"),
      workspaceDir: tmpDir,
      body: "no visible sends",
      isFallbackRetry: false,
      resolvedThinkLevel: "medium",
      timeoutMs: 1_000,
      runId: "run-visible-send-policy",
      opts: {
    ...
    expectMockArgFields(runEmbeddedAgentMock, { visibleSendPolicy: "deny" });
  });
